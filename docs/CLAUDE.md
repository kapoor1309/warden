...
- **Test at the agentic boundary — the strongest, deterministic proof.** A prompt
  payload may not land on an extract-only worker, and a robust model *ignoring* an
  off-surface injection is expected, not evidence the defense works. So also assume the
  worst case — the worker is ALREADY compromised — and have it send malicious,
  in-policy-looking records straight through Band. No model refusal is involved; the
  deterministic layer (Warden's invariants + the signed pay-gate) must block every one.
  `demo/run_boundary_attacks.py` is the matrix: payee redirect, cross-vendor PO swap,
  amount inflation, skipped review, out-of-role, and a forged Payer sign-off — one per
  invariant + the gate. Pull the malicious *content* (e.g. the attacker IBAN) from the
  benchmarks so even the boundary attack isn't self-authored.
